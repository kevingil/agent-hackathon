...
class OrderService:
    ...
    @staticmethod
    def add_item_to_cart(order_id: int, stock_item_id: int, quantity: int = 1) -> Order:
    ...
    def update_cart_item_quantity(
        order_id: int, item_id: int, new_quantity: int
    ) -> Order:
        ...
        try:
            ...
            order_item = next(
                (item for item in order.items if item.id == item_id), None
            )
